...
ready to use in `topGO`. Table \@ref(tab:algos) presents the compatibility table between 
...
The `elim` and `weight` algorithms were introduced in [@Alexa2006]. The default algorithm
used by the `topGO` package is a mixture between the `elim` and the `weight` algorithms and
it will be referred as `weight01`. The `parentChild` algorithm was introduced by [@Grossmann2007].

We assume the user has a good understanding of GO, see [@GOC01], and is familiar with gene set enrichment tests. Also this document requires basic knowledge of R language.
...
1. *Running the enrichment tests:* Using the object created in the first step the user can perform
  enrichment analysis using any feasible mixture of statistical tests and methods that deal with the GO topology.
  
1. *Analysis of the results:* The results obtained in the second step are analysed using 
...
The microarray used in the experiment is the `r annotation(ALL)` from Affymetrix, as we can see 
...
The function assumes that the provided argument is a named vector of $p$-values. With the help of this function we can see that there are `r sum(topDiffGenes(geneList))` genes with a raw $p$-value less
...
gene-to-GO mappings from the `affyLib` object. Section \@ref(sec:topGOdata) describes the
...
the methods and statistical tests is shown in Table \@ref(tab:algos)).
...
For accessing the GO term's $p$-values from a `topGOresult` object the user should use the `score` functions. As a simple example, we look at the differences between the results of the `classic` and the `elim` methods in the case of the Kolmogorov-Smirnov test. The `elim` method was design to be 
...
## The `topGOdata` object {#sec:topGOdata}
...
Table \@ref(tab:algos), each of them having specific requirements.  
...
will work, as seen in Table \@ref(tab:algos). In the case of a mismatch the function will throw an error.
