...
We can see in Figure \@ref(fig:scatterClassicElim) that there are indeed differences between
...
To demonstrate the package functionality we will use the ALL (Acute Lymphoblastic Leukemia) gene expression data from [@Chiaretti04]. The dataset consists of $128$ microarrays from different patients with ALL measured using the HGU95aV2 Affymetrix chip.
...
- A mapping between gene identifiers and GO terms. In most cases this mapping is directly available
  ...
  In this case the user just needs to specify the name of the annotation to be used. For example, the annotation package needed for the ALL dataset is `r affyLib`.
...
```{r whichProbes, fig.cap="Scatter plot of FDR adjusted $p$-values against variance of probes. Points below the horizontal line are significant probes.", fig.width=9, fig.height=7}
library(lattice)
...
Figure \@ref(fig:whichProbes) shows for the three groups of probes the adjusted $p$-values and the gene-wise variance. 
...
`classicScore` class, see Figure~\@ref(fig:topGOclasses) which will let us run the `classic`
algorithm.
...
%This time we used the class `classicScore`. This is done since the KS test needs scores of all genes and
%in this case the *representation* of a group of genes (GO term) is different. 
...
Both `getSigGroups` and `runTest` functions return an object of type `topGOresult`, and most of
...
statistics. Table~\@ref(tab:GOresults) shows the informations included in the `data.frame`.
...
reported by `classic` and `weight` methods. There are two functions available. The `showSigOfNodes`
...
\caption{The subgraph induced by the top 5 GO terms identified by the `weight` algorithm for
  scoring GO terms for enrichment. Boxes indicate the 5 most significant terms. Box color represents the
  relative significance, ranging from dark red (most significant) to light yellow (least significant).
  Black arrows indicate *is-a* relationships and red arrows *part-of* relationships.}
\end{figure}
