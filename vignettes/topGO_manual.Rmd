...
output: 
  BiocStyle::html_document:
    toc_float: true
vignette: >
  %\VignetteIndexEntry{Gene set enrichment analysis with topGO}
  %\VignetteEncoding{UTF-8}  
  %\VignettePackage{topGO}
  %\VignetteKeywords{GeneExpression, Transcriptomics, GeneSetEnrichment, GO, 
  Annotation, Pathways, SystemsBiology, Microarray, Sequencing, Visualization, 
  Software}
...
The {\sf elim} and {\sf weight} algorithms were introduced in~@Alexa2006. The default algorithm
used by the `topGO` package is a mixture between the {\sf elim} and the {\sf weight} algorithms and
it will be referred as {\sf weight01}. The {\sf parentChild} algorithm was introduced by~@Grossmann2007.

We assume the user has a good understanding of GO, see~@GOC01, and is familiar with gene set enrichment 
...
The `topGO` package is available from the Bioconductor repository at http://www.bioconductor.org
...
\begin{enumerate}
-  {\it Data preparation:} List of genes identifiers, gene scores, list of differentially expressed genes
...
-  {\it Running the enrichment tests:} Using the object created in the first step the user can perform
  enrichment analysis using any feasible mixture of statistical tests and methods that deal with the GO topology.
  
-  {\it Analysis of the results:} The results obtained in the second step are analysed using 
...
example of a list of gene $p$-values is provided by the `geneList` object.
...
The `geneList` data is based on a differential expression analysis of the ALL(Acute Lymphoblastic Leukemia)
dataset that was extensively studied in the literature on microarray analysis~@Chiaretti04.
...
The microarray used in the experiment is the {\tt `r annotation(ALL)`} from Affymetrix, as we can see 
...
function we can see that there are `r sum(topDiffGenes(geneList))` genes with a raw $p$-value less
than $0.01$ out of a total of `r length(geneList)` genes.
...
gene-to-GO mappings from the `affyLib` object. Section~\@ref(subsec:topGOdata) describes the
...
`runTest` returns an object of class {\tt topGOresult}. A short summary of this object is shown below.
...
by the {\sf elim} algorithm, see `orderBy` argument. The data frame includes some statistics on the
...
For accessing the GO term's $p$-values from a `topGOresult` object the user should use the {\tt score}
functions. As a simple example, we look at the differences between the results of the {\sf classic} and
the {\sf elim} methods in the case of the Kolmogorov-Smirnov test. The `elim` method was design to be 
more conservative then the `classic` method and therefore one expects the $p$-values returned by the
...
data from @Chiaretti04. The dataset consists of $128$ microarrays from different patients with ALL
...
When the `topGO` package is loaded three environments {\tt GOBPTerm, GOMFTerm} and {\tt GOCCTerm}
...
variability across samples. Package `genefilter` provides tools for filtering genes. In this 
...
eset <- ALL[selProbes, ]
``` 
The filter selects only $`r nrow(eset)`$ probesets out of $`r nrow(ALL)`$ probesets available 
on the $`r annotation(ALL)`$ array.
...
the ALL dataset we have $`r length(featureNames(eset))`$ feasible genes, the ones that were not
removed by the filtering procedure. 


\subsection{The `topGOdata` object}
{#subsec:topGOdata}
...
-  The GO hierarchical structure. This structure is obtained from the `GO.db` package. At the moment
  `topGO` supports only the ontology definition provided by {\tt GO.db}.

\end{itemize}

We further describe the arguments of the `initialize` function ({\tt new}) used to construct an instance
of this {\it data container} object.

\begin{description}
- [{\tt ontology:}] character string specifying the ontology of interest (BP, MF or CC)
  
- [{\tt description:}] character string containing a short description of the study [optional].
  
- [{\tt allGenes:}] named vector of type numeric or factor. The names attribute contains the genes
  identifiers. The genes listed in this object define the gene universe.
  
- [{\tt geneSelectionFun:}] function to specify which genes are interesting based on the gene scores. 
  It should be present iff the `allGenes` object is of type numeric.

- [{\tt nodeSize:}] an integer larger or equal to $1$. This parameter is used to prune the GO hierarchy
  from the terms which have less than `nodeSize` annotated genes (after the true path rule is applied).
  
- [{\tt annotationFun:}] function which maps genes identifiers to GO terms. There are a couple of annotation
  ...
  \begin{description}
  - [{\tt annFUN.db}] this function is intended to be used as long as the chip used by the user has an
    annotation package available in Bioconductor.
    
  - [{\tt annFUN.org}] this function is using the mappings from the "org.XX.XX" annotation packages.
  ...
  - [{\tt annFUN.gene2GO}] this function is used when the annotations are provided as a gene-to-GOs mapping.
    
  - [{\tt annFUN.GO2gene}] this function is used when the annotations are provided as a GO-to-genes mapping.
    
  - [{\tt annFUN.file}] this function will read the annotationsof the type gene2GO or GO2genes
    from a text file.    
  \end{description}

- [{\tt ...:}] list of arguments to be passed to the {\tt annotationFun}
\end{description}
...
The `geneList` object is a named factor that indicates which genes are interesting and which not. It
...
To build the `topGOdata` object, we will use the MF ontology. The mapping is given by the {\tt geneID2GO}
list which will be used with the `annFUN.gene2GO` function.
...
define which are the interesting genes, in the `topGOdata` object is that one can apply
...
There are $`r table(y)[1]`$ B-cell ALL samples and $`r table(y)[1]`$ T-cell ALL samples in
the dataset. A two-sided $t$-test can by applied using the function `getPvalues` (a wrapping function for 
...
of the arguments used by the `initialize` function see Section~\@ref(subsec:sigGenes)
...
There are a total of $`r nrow(ALL)`$ probes on the $`r annotation(ALL)`$ chip. One assumes that only
...
The filtering procedure removes $`r tt[3]`$ probes which is a very large percentage of probes (more than 
...
\section{Working with the `topGOdata` object}
...
\begin{itemize}
-  Tests based on gene {\bf counts}. This is the most popular family of tests, given that 
  it only requires the presence of a list of interesting genes and nothing more. Tests like 
  Fisher's exact test, Hypergeometric test and binomial test belong to this family. @Draghici2006
-  Tests based on gene {\bf scores} or gene {\bf ranks}. It includes 
  Kolmogorov-Smirnov like tests (also known as GSEA), Gentleman's Category, $t$-test, etc. @Ackermann2009
-  Tests based on gene {\bf expression}. Tests like Goeman's globaltest or GlobalAncova separates
  from the others since they work directly on the expression matrix. @Goeman2007
\end{itemize}
...
{\sf classic} algorithm. The two classes defined for this algorithm are `classicCount` and
{\tt classicScore}. If an object of this class is given as a argument to `getSigGroups()` than 
...
%% `getSigGroups` returns an object of class {\tt topGOresult}.
...
`classicScore` class, see Figure~\@ref(fig:topGOclasses) which will let us run the {\sf classic}
algorithm.
...
-  For the methods that account for the GO topology like {\sf elim} and {\sf weight}, the problem of
...
%%In fact the `runTest` function is a warping of the `getSigGroups` and the 
%%initialisation of a `groupStats` object functions. 
...
The `algorithm` argument is optional and if not specified the {\sf weight01} method will be used. 
...
algorithm based on the available `groupStats` classes, then it would be not possible to use the
`runTest` function. 
...
Both `getSigGroups` and `runTest` functions return an object of type {\tt topGOresult}, and most of
the following functions work with this object.

\subsection{The `topGOresult` object}
...
the `topGOdata` object is not carried over this object, and both of these objects will be needed 
...
By default, the `score` function does not warranty the order in which the $p$-values are returned, 
...
However, the `score` method has a parameter, {\tt whichGO}, which takes a list of GO identifiers and
...
Basic information on input data can be accessed using the `geneData` function. The number of annotated genes, 
...
{\tt rankOf}, etc. This is the price paid for flexibility of specifying different number of `topGOresults` 
...
We can see in Figure~\@ref(fig:geneDensityDiff) that the genes annotated to `r goID` have low ranks
...
will plot the induced subgraph to the current graphic device. The `printGraph` is a warping function
of `showSigOfNodes` and will save the resulting graph into a PDF or PS file. 
...
\addcontentsline{toc}{section}{References} {#references}
