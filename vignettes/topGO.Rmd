...
`r BiocStyle::Biocpkg("topGO")` package was designed to work with various test statistics and various algorithms
...
{\tt topGOdata}. The basic interface, which provides the core of the testing procedure in `r BiocStyle::Biocpkg("topGO")`, 
...
the test statistic. We used the `GOFisherTest` function which is available in the `r BiocStyle::Biocpkg("topGO")` package
...
case `classic` and then runs the algorithm (see the second line). The slot {\tt testStatistic}
contains the test statistic function. In the above example `GOFisherTest` function which implements
Fisher's exact test and is available in the `r BiocStyle::Biocpkg("topGO")` package was used. A user can define his own test
...
%Please note that there are extra parameters for `elim` and {\sf weight}, but we won't discuss them at this point. 
...
The mechanism presented above for `classic` also hold for `elim` and {\sf weight}. The user
...
-  For the methods that account for the GO topology like `elim` and {\sf weight}, the problem of
...
Thus, one can specify different cutoffs for the `elim` method, or arguments for the {\sf weight} method. 
...
To see how this work lets compute the correlation between the $p$-values of the `classic` and {\sf weight}
methods:
...
reported by `classic` and {\sf weight} methods. There are two functions available. The {\tt showSigOfNodes}
