...
Besides providing an easy to use set of functions for performing GO enrichment analysis, it also enables the user to easily implement new statistical tests or new algorithms that deal with
the GO graph structure. 
...
ready to use in `r BiocStyle::Biocpkg("topGO")`. 
...
used by the `r BiocStyle::Biocpkg("topGO")` package is a mixture between the `elim` and the `weight` algorithms and it will be referred as `weight01`. 
